...
#         dtype = np.complex128
    ...
    for ttx in X:
        n,r = nr(ttx)
        rx.append(r)
    rx = np.transpose(np.array(rx))
    crx = np.empty((nx, d), dtype=np.object) # to store cores
    i = 0
    for ttx in X:
#         v = tt.tensor.to_list(ttx) # from teneva just ttx
        v = ttx
        j = 0
        for w in v:
            crx[i, j] = w
    ...
    if y0 is None:
        ...
        ry[0] = 1
        y = teneva.rand(n, ry) # from teneva
        wasrand = True
...
#         _, ry = nr(y).copy()
    ...
    _,rz = nr(z)
#     z = tt.tensor.to_list(z) # from teneva just z
    z = z
    _,ry = nr(y)
#     cry = tt.tensor.to_list(y) # from teneva just y
    cry = y
    ...
    for i in range(0, d - 1):
        ...
        if wasrand:
            ...
            if Ry[i + 1].shape[0] <= Ry[i + 1].shape[1]:
                curind = arange(Ry[i + 1].shape[0])
            else:
                curind, _ = maxvol(Ry[i + 1])
        ...
        if dirn > 0 and i < d - 1:
            ...
            if kickrank > 0:
                ...
                Rz[i + 1] = np.dot(Rz[i], reshape(zz,
                                                  (rz[i], n[i] * rz[i + 1])))
                Rz[i + 1] = reshape(Rz[i + 1], (rz[i] * n[i], rz[i + 1]))
                if Rz[i + 1].shape[0] <= Rz[i + 1].shape[1]:
                    curind = arange(Rz[i + 1].shape[0])
                else:
                    curind, _ = maxvol(Rz[i + 1])
                Ryz[i + 1] = Ryz[i + 1][curind, :]
                Rz[i + 1] = Rz[i + 1][curind, :]
                # Interface matrices for X
                for j in xrange(nx):
            ...
            if Ry[i].T.shape[0] <= Ry[i].T.shape[1]:
                curind = arange(Ry[i].T.shape[0])
            else:
                curind, _ = maxvol(Ry[i].T)
            Ry[i] = Ry[i][:, curind]
            # Interface matrices for X
            for j in xrange(nx):
            ...
            if kickrank > 0:
                ...
                if Rz[i].T.shape[0] <= Rz[i].T.shape[1]:
                    curind = arange(Rz[i].T.shape[0])
                else:
                    curind, _ = maxvol(Rz[i].T)
                Ryz[i] = Ryz[i][:, curind]
                Rz[i] = Rz[i][:, curind]
                # Interface matrices for X
                for j in xrange(nx):
...
#     y = tt.tensor.from_list(cry) # from teneva cry
    y = cry
    return y